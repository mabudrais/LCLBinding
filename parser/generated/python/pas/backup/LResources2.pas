...
type
  ...
  TMreader=   Class(TReader)
     function FindMethod(ARoot: TComponent; const AMethodName: string
       ): CodePointer; override;
      procedure SetMethodPropertyEvent(Reader: TReader; Instance: TPersistent;
...
function TMreader.FindMethod(ARoot: TComponent; const AMethodName: string
  ): CodePointer;
begin
  Result:=Nil;
end;

procedure TMreader.SetMethodPropertyEvent(Reader: TReader; Instance: TPersistent;
    PropInfo: PPropInfo; const TheMethodName: string;
    var Handled: boolean) ;
begin
  ObjectSData:=ObjectSData+'*Event'+'*'+PropInfo^.Name+'*'+TheMethodName+'*'+Integer(Pointer(Instance)).ToString();
  Handled:=True;
end;

procedure TMreader.SetNameEvent(Reader: TReader; Component: TComponent;
  var Name: string);
begin
  ObjectSData:=ObjectSData+'*Pointer'+'*NON*'+Name+'*'+Integer(Pointer(Component)).ToString();
end;
...
begin
   ObjectSData:=ObjectSData+'*Pointer'+'*'+Name+'*'+Integer(Pointer(Component)).ToString();
  MakeValidIdentifier;
...
begin
  ...
  MR:=(Result as TMreader);
  Result.OnSetMethodProperty:=@MR.SetMethodPropertyEvent;
  //Result.OnSetName:=@MR.SetNameEvent;
  DestroyDriver:=false;
